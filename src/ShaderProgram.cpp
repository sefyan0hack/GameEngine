...
auto ShaderProgram::checkProgramLinkStatus(const ShaderProgram& material) -> void
{
    ...
    GLint success = GetProgramInfo(id, GL_LINK_STATUS);

    if (!success) {
        GLsizei  infologlength = GetProgramInfo(id, GL_INFO_LOG_LENGTH);
        if(infologlength > 0){
...
{
    GLint count = GetProgramInfo(m_Id, GL_ACTIVE_UNIFORMS);

    if(count < 0){
        Debug::Print("program id is not a value generated by OpenGL.");
    }
...
{
    GLint count = GetProgramInfo(m_Id, GL_ACTIVE_ATTRIBUTES);

    if(count < 0){
        Debug::Print("error id {}, GetProgramInfo return {}", m_Id, count);
    }
...
{
    GLint max_len = GetProgramInfo(m_Id, GL_ACTIVE_UNIFORM_MAX_LENGTH);
        
    if(max_len == 0) {
        Debug::Print("error id {}, GetProgramInfo return {}", m_Id, max_len);
        return;
    }
...
{
    GLint max_len = GetProgramInfo(m_Id, GL_ACTIVE_ATTRIBUTE_MAX_LENGTH);
    if(max_len == 0) {
        Debug::Print("max_len is not valid max_len: {}", max_len);
        return;
    }
...
auto ShaderProgram::GetProgramInfo(GLint id, GLenum what) -> GLint
{
