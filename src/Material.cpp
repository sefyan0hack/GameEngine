...
Material::~Material()
{
    if(gl::IsProgram(m_Id) == GL_TRUE) gl::DeleteProgram(m_Id);
}
...
{
    m_Previd = Current_Program();
    if(m_Id != 0 && gl::IsProgram(m_Id) == GL_TRUE){
...
{
    Expect(gl::IsProgram(m_Id) == GL_TRUE , "program id {} is not a value generated by OpenGL", (GLuint)m_Id);
    GLint count;
